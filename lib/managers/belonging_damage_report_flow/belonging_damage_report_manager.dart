...
class BelongingDamageReportManager implements BelongingDamageReportInterface {
  ...
  Future<InteractionResponse?> postInteraction() async {
    ...
    if (report == null) {
      debugPrint(
        '[BelongingDamageReportManager] buildBelongingReport() returned null, aborting send',
      );
      return null;
    }
    ...
    if (interactionResponseModel != null) {
      debugPrint(
        '[BelongingDamageReportManager] ✅ Interaction posted successfully.',
      );
      // you can keep your questionnaire logging here
      formProvider.clearStateOfValues();
    } else {
      debugPrint(
        '[BelongingDamageReportManager] ❌ No response (probably offline).',
      );
  ...
  BelongingDamageReport? buildBelongingReport() {
    ...
    debugPrint(
      "✅ formProvider.impactedAreaType: '${formProvider.impactedAreaType}'",
    );
    ...
    try {
      ...
      final String impactType =
          formProvider.apiImpactType; // "square-meters" | "units"
      final int? impactValueInt =
          formProvider.apiImpactValueOrNull; // already m²/units as INT
