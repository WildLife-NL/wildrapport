...
void main() {
  ...
  group('AnimalManager', () {
    ...
    test('should filter animals based on current filter', () async {
      ...
      when(mockFilterManager.filterAnimalsAlphabetically(any)).thenReturn(mockFilteredAnimals);
      ...
      // Assert
      verify(mockFilterManager.filterAnimalsAlphabetically(any)).called(greaterThan(0));
      expect(animals, isNotEmpty);
    });
    ...
    test('should remove listeners correctly', () {
      ...
      // Assert
      expect(listenerCalled, false); // Listener should not be called after removal
    });
    
    test('should return unfiltered animals when filter is not recognized', () async {
      // Arrange
      AnimalHelpers.setupSpeciesApiResponse(mockSpeciesApi);
    ...
    test('should return most viewed animals when most viewed filter is selected', () async {
      // Arrange
      AnimalHelpers.setupSpeciesApiResponse(mockSpeciesApi);
    ...
    test('should handle empty search results', () async {
      ...
      when(mockFilterManager.searchAnimals(any, 'NonExistentAnimal'))
          .thenReturn([]);
      ...
      // Assert
      expect(animals, isEmpty);
      verify(mockFilterManager.searchAnimals(any, 'NonExistentAnimal')).called(1);
    });
    ...
    test('should prioritize search over filter when both are set', () async {
      ...
      when(mockFilterManager.filterAnimalsAlphabetically(any))
          .thenReturn([/* different results */]);
      when(mockFilterManager.searchAnimals(any, 'Wolf'))
    ...
    test('should search animals using contains (partial match)', () async {
      ...
      final partialMatches = allAnimals.where((a) => 
          a.animalName.toLowerCase().contains('wolf')).toList();
      
      when(mockFilterManager.searchAnimals(any, 'olf')).thenReturn(partialMatches);
      ...
      expect(searchResults.length, 2); // Should match both "Wolf" and "Red Wolf"
    ...
    test('should handle case-insensitive search', () async {
      ...
      when(mockFilterManager.searchAnimals(any, 'WOLF')).thenReturn(searchResults);
