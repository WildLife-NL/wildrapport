...
void main() {
  ...
  group('TrackingCacheManager - Storage Stress Tests', () {
    ...
    test('should handle 100 cached readings', () async {
      ...
      print('Storage size: ${storageSize} bytes (${(storageSize / 1024).toStringAsFixed(2)} KB)');
    ...
    test('should handle 1,000 cached readings', () async {
      ...
      final storageSize = cachedJson!.join('').length;
      print('Storage size: ${storageSize} bytes (${(storageSize / 1024).toStringAsFixed(2)} KB)');
      print('Average size per reading: ${(storageSize / 1000).toStringAsFixed(2)} bytes');
    ...
    test('should handle 10,000 cached readings', () async {
      ...
      for (int i = 0; i < 10000; i++) {
        await cacheManager.cacheReading(TrackingReading(
          latitude: 52.0 + (i * 0.0001),
          longitude: 5.0 + (i * 0.0001),
          timestampUtc: DateTime.utc(2025, 11, 26, 12 + (i ~/ 3600), (i % 3600) ~/ 60, i % 60),
        ));
      }

      final cacheTime = DateTime.now().difference(startTime);
      print('Time to cache 10,000 readings: ${cacheTime.inMilliseconds}ms (${(cacheTime.inMilliseconds / 1000).toStringAsFixed(2)}s)');

      // Calculate storage size
      final prefs = await SharedPreferences.getInstance();
      final cachedJson = prefs.getStringList('cached_tracking_readings');
      final storageSize = cachedJson!.join('').length;
      print('Storage size: ${storageSize} bytes (${(storageSize / 1024).toStringAsFixed(2)} KB or ${(storageSize / (1024 * 1024)).toStringAsFixed(2)} MB)');
      print('Average size per reading: ${(storageSize / 10000).toStringAsFixed(2)} bytes');
    ...
    test('should measure retrieval performance for large cache', () async {
      ...
      print('Average retrieval time per reading: ${(retrievalTime.inMilliseconds / cached.length).toStringAsFixed(4)}ms');
    ...
    test('should test realistic scenario: 1 week offline at 10s intervals', () async {
      ...
      for (int i = 0; i < readingsPerWeek; i++) {
        await cacheManager.cacheReading(TrackingReading(
          ...
          timestampUtc: DateTime.utc(2025, 11, 26).add(Duration(seconds: i * 10)),
        ));
      }
      
      final cacheTime = DateTime.now().difference(startTime);
      print('Time to cache: ${(cacheTime.inMilliseconds / 1000).toStringAsFixed(2)}s');
      ...
      print('Storage used: ${(storageSize / (1024 * 1024)).toStringAsFixed(2)} MB');
      print('Average per reading: ${(storageSize / readingsPerWeek).toStringAsFixed(2)} bytes');
    ...
    test('should test edge case: maximum SharedPreferences capacity', () async {
      ...
      print('Note: SharedPreferences on Android has a practical limit of ~1-2 MB per key');
      print('      On iOS, the limit is higher but still constrained by device storage\n');
      ...
      try {
        ...
        for (int i = 0; i < 15000; i++) {
          ...
          if (i > 0 && i % 1000 == 0) {
            ...
            final cachedJson = prefs.getStringList('cached_tracking_readings');
            final storageSize = cachedJson!.join('').length;
            print('${i} readings: ${(storageSize / (1024 * 1024)).toStringAsFixed(2)} MB');
          }
        }
      } catch (e) {
      ...
      if (!limitReached) {
        ...
        print('Total storage: ${(storageSize / (1024 * 1024)).toStringAsFixed(2)} MB');
      }
    ...
    test('should provide storage recommendations', () {
      ...
      final scenarios = [
        ...
        {'duration': '6 hours', 'readings': 6 * 60 * 6, 'name': '6 hours offline'},
        {'duration': '1 day', 'readings': 6 * 60 * 24, 'name': '1 day offline'},
        {'duration': '3 days', 'readings': 6 * 60 * 24 * 3, 'name': '3 days offline'},
        {'duration': '1 week', 'readings': 6 * 60 * 24 * 7, 'name': '1 week offline'},
        {'duration': '1 month', 'readings': 6 * 60 * 24 * 30, 'name': '1 month offline'},
      ];
      ...
      for (var scenario in scenarios) {
        ...
        if (storageMB < 1) {
          ...
          print('     Consider implementing cache size limits or using database');
        }
        print('');
      }
      ...
      print('3. Consider using SQLite database for large caches (>10,000 readings)');
