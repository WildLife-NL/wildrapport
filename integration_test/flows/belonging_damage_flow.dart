...
void runTests() {
  group('End-to-end tests for Gewasschade reporting flow', () {
    testWidgets(
      'User can navigate and complete the Gewasschade reporting flow',
      (WidgetTester tester) async {
        ...
        expect(
          token,
          isNotNull,
          reason: 'Bearer token must be set for API authentication',
        );
        ...
        // Debug widget tree
        debugPrint(
          tester.allWidgets.map((w) => w.toString()).toList().join('\n'),
        );

        // Verify OverzichtScreen is displayed
        expect(
          find.byType(OverzichtScreen),
          findsOneWidget,
          reason:
              'OverzichtScreen should be displayed after login or app start',
        );

        // Step 1: Verify and tap 'Rapporteren'
        expect(
          find.byKey(const Key('rapporteren_button')),
          findsOneWidget,
          reason: 'Rapporteren button should be visible',
        );
        ...
        final tapPositionDamage =
            damageSliderBox.topLeft +
            Offset(damageDragOffset, damageSliderBox.height / 2);
        ...
        final futureDamageSlider = find.byKey(
          const Key('estimated-future-damage'),
        );
        final futureDamageSliderBox = tester.getRect(futureDamageSlider);
        final futureDamageDragOffset =
            (3350 / 10000) * futureDamageSliderBox.width;
        final tapPositionFutureDamage =
            futureDamageSliderBox.topLeft +
            Offset(futureDamageDragOffset, futureDamageSliderBox.height / 2);
